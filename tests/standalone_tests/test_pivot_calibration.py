"""
Testing pivot calibrator works.
...
def rand_rot(rng): # generate random rotation with det not 0
    M = rng.normal(size=(3,3))
    Q, _ = np.linalg.qr(M)
    if np.linalg.det(Q) < 0: Q[:,0] *= -1
    return Q

def _rms(A): # calculates RMS error
    return float(np.sqrt(np.mean(np.sum(A*A, axis=-1))))

"""
Uses defined data to ensure calibrated transformations lead back to original.
"""
...
"""
Testing using self-defined data.
"""
